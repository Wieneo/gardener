...
func (r *Reconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		...
		if err := r.patchStatus(ctx, log, shoot, shootConditions.ConditionTypes(), updatedConditions, shootConstraints.ConstraintTypes(), updatedConstraints); err != nil {
...
func (r *Reconciler) patchStatus(ctx context.Context, log logr.Logger, shoot *gardencorev1beta1.Shoot, conditionTypes []gardencorev1beta1.ConditionType, conditions []gardencorev1beta1.Condition, constraintTypes []gardencorev1beta1.ConditionType, constraints []gardencorev1beta1.Condition) error {
	...
	mergedConditions := v1beta1helper.BuildConditions(shoot.Status.Conditions, conditions, conditionTypes)
	mergedConstraints := v1beta1helper.BuildConditions(shoot.Status.Constraints, constraints, constraintTypes)

	// Update Shoot status (conditions, constraints) only if necessary
	if !v1beta1helper.ConditionsNeedUpdate(conditions, mergedConditions) && !v1beta1helper.ConditionsNeedUpdate(constraints, mergedConstraints) {
		return nil
	}
